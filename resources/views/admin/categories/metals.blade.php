...
@section('title', 'Категория: металл')
...
@section('content')
    <div id="CategoryPage">
        ...
        <div class="container">
            <div class="row  mt-5">
                <div class="col-8">
                    <h2 class="text-primary">Категория: металл</h2>
                </div>
        ...
        const CategoryFunctions = {
            ...
            methods: {
                ...
                async Add() {
                    ...
                    const response = await fetch('{{route('MetalAdd')}}', {
                        method: 'post',
                        headers: {
                ...
                //---Удаление категории
                async Delete() {
                    const response = await fetch('{{route('MetalDelete')}}', {
                        method: 'post',
                        headers: {
                            'X-CSRF-TOKEN': '{{csrf_token()}}',
                ...
                async Edit() {
                    ...
                    const response = await fetch('{{route('MetalEdit')}}', {
                        method: 'post',
                        headers: {
                            'X-CSRF-TOKEN': '{{csrf_token()}}',
                        },
